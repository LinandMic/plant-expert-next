...
export function normalizePlantFinderFilters(input = {}) {
  ...
  // offset is the "Charger plus" page cursor — how many published rows
  // (already matching this same query/filters) to skip before this page's
  // `limit` rows. Never negative, never forwarded raw: an invalid/missing
  // value always falls back to 0 (the first page), the same choke-point
  // discipline as every other filter value here.
  const rawOffset = Number.isFinite(input.offset) ? Math.floor(input.offset) : 0;
  const offset = Math.max(rawOffset, 0);

  return { query, plantType, sun, heightCategory, limit, offset };
}
...
// formatResultCount(count, t) -> "1 plante trouvée" | "N plantes trouvées"
export function formatResultCount(count, t) {
  return count === 1 ? t("finder.resultCountOne") : t("finder.resultCountOther", { count });
}
