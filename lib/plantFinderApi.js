...
const LIST_SELECT = [
  ...
  "image_license",
  "image_source_url",
  // plant_taxa.id: only added for LIST_SELECT (fetchPublishedPlantBySlug —
  // the Plant Detail single-row path), never for the search RPC's own flat
  // row shape (rpcRowToPlant below, unchanged) — search stays untouched.
  // Surfaces as taxon.id on the mapped Plant, which the "add to garden"
  // flow (components/AddToGardenModal.js) uses to preserve canonical taxon
  // identity on the inserted plants row (see
  // supabase/migrations/20260915100000_add_catalog_plant_link_v1.sql) —
  // never a new query, just one more already-existing column on the
  // resource this page already embeds.
  "plant_taxa ( id, canonical_name, family, genus, plant_common_names ( name, locale, is_preferred ) )",
].join(", ");
...
//
// preferredCommonNameFr/preferredCommonNameEn: the taxon's is_preferred
// plant_common_names row per locale (see supabase/migrations/
// 20260911140000_add_plant_common_names_v1.sql), never
// plant_catalog.common_name (kept above as `commonName` only for backward
// compatibility with any code still reading it — it is never populated by
// either caller of this function and is not the multilingual source).
// Resolved by each caller BEFORE calling baseRowToPlant (one already has
// the RPC's own flat columns, the other reads an embedded PostgREST
// resource) so this shared mapper stays agnostic to where they came from.
function baseRowToPlant(row, taxon, preferredCommonNameFr, preferredCommonNameEn) {
  return {
    id: row.id,
    slug: row.slug,
    ...
    preferredCommonNameFr: preferredCommonNameFr ?? null,
    preferredCommonNameEn: preferredCommonNameEn ?? null,
...
// preferredNamesFromEmbedded(commonNames) -> { fr, en }
// commonNames is the array PostgREST embeds at plant_taxa.plant_common_names
// (LIST_SELECT above) — every locale's name for the taxon, preferred or
// not. At most one is_preferred=true row can exist per locale (enforced by
// plant_common_names_one_preferred_per_taxon_locale, a live unique partial
// index), so `.find()` never has to pick among candidates; an empty/absent
// array (a taxon with no common names yet) yields { fr: null, en: null },
// never a fabricated name.
function preferredNamesFromEmbedded(commonNames) {
  const rows = Array.isArray(commonNames) ? commonNames : [];
  const fr = rows.find((r) => r.locale === "fr" && r.is_preferred);
  const en = rows.find((r) => r.locale === "en" && r.is_preferred);
  return { fr: fr ? fr.name : null, en: en ? en.name : null };
}

export function rowToPlant(row) {
  const { fr, en } = preferredNamesFromEmbedded(row.plant_taxa ? row.plant_taxa.plant_common_names : null);
  return baseRowToPlant(
    row,
    row.plant_taxa
      ? {
          id: row.plant_taxa.id,
...
//
// preferred_common_name_fr/preferred_common_name_en: added to the RPC's
// row shape by supabase/migrations/20260914100000_add_common_name_display_v1.sql
// via an indexed lateral lookup on the same taxon_id — no second query. Not
// yet applied live this round (see that migration's own header); until it
// is, these columns are simply absent from every row and `?? null` below
// means callers see the same fallback-safe null they'd see once it is
// applied, never a crash on a missing key.
function rpcRowToPlant(row) {
  return baseRowToPlant(
    ...
    row.preferred_common_name_fr ?? null,
    row.preferred_common_name_en ?? null
  );
}
...
// fetchCatalogPlantById(catalogPlantId) -> Plant | null
// Same shape/query/publication_status guard as fetchPublishedPlantBySlug
// above (byte-for-byte the same LIST_SELECT + rowToPlant mapping — no
// duplicated field list to drift out of sync) — only the lookup key
// differs (plant_catalog.id, not slug). Added for the "catalog plant
// inside My Garden" detail view (components/CatalogPlantGardenDetail.js),
// which has a garden plant's catalog_plant_id but not its slug. null means
// "no published plant at this id" — a catalog entry that was published
// when added to a garden but has since been archived/unpublished is
// indistinguishable from one that never existed, same non-leaking
// contract as the slug lookup; the caller falls back to the garden
// plant's own add-time snapshot rather than breaking.
export async function fetchCatalogPlantById(catalogPlantId) {
  if (!catalogPlantId) return null;
