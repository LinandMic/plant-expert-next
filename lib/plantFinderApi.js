...
// row is trimmed off before returning; it is never rendered. This avoids a
// separate COUNT query and never depends on limit/hasMore being confused
// with each other.
export async function searchPublishedPlants(params) {
  ...
  return { plants: rows.slice(0, limit).map(rowToPlant), hasMore };
}
