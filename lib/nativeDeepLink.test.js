...
test("accepts a supported route on the configured origin, preserving the recovery hash", () => {
  ...
  assert.deepEqual(result, { localPath: "/reset-password.html#access_token=abc&refresh_token=def&type=recovery" });
});
...
test("accepts a supported route with query params too", () => {
  ...
  assert.deepEqual(result, { localPath: "/reset-password.html?foo=bar" });
});

test("maps the trusted /reset-password URL to /reset-password.html internally, preserving the recovery hash exactly", () => {
  const result = resolveNativeDeepLink(
    "https://almeo.app/reset-password#access_token=abc&refresh_token=def&type=recovery",
    "https://almeo.app",
  );
  assert.deepEqual(result, { localPath: "/reset-password.html#access_token=abc&refresh_token=def&type=recovery" });
});

test("preserves query and hash together, byte for byte, in the .html local path", () => {
  const result = resolveNativeDeepLink(
    `${WEB_ORIGIN}/reset-password?a=1&b=x%20y#access_token=t%2Bk&type=recovery`,
    WEB_ORIGIN,
  );
  assert.deepEqual(result, { localPath: "/reset-password.html?a=1&b=x%20y#access_token=t%2Bk&type=recovery" });
});

test("the .html page is internal only: an external /reset-password.html is NOT a trusted route", () => {
  assert.equal(resolveNativeDeepLink(`${WEB_ORIGIN}/reset-password.html`, WEB_ORIGIN), null);
  assert.equal(resolveNativeDeepLink(`${WEB_ORIGIN}/reset-password/`, WEB_ORIGIN), null);
});
...
test("first delivery of a supported URL is handled and navigates once", () => {
  ...
  assert.deepEqual(navigated, ["/reset-password.html#access_token=tokA&refresh_token=refA&type=recovery"]);
});
...
test("cold start delivers the same URL twice in one document (appUrlOpen + getLaunchUrl): one navigation", () => {
  ...
  assert.deepEqual(navigated, ["/reset-password.html"]);
});
