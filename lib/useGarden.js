...
import { FREE_GARDEN_LIMIT_REACHED, monetizationErrorCode } from "./monetizationErrors";
...
// source/commonName/latinName/family/category/catalogPlantId/taxonId/
// preferredCommonNameFr/preferredCommonNameEn: added for catalog-sourced
// garden plants (source='catalog', see components/AddToGardenModal.js +
// gardenApi.insertCatalogPlant). ai_identification rows simply never set
// these DB columns (source defaults to 'ai_identification', the rest stay
// null) — their existing display path (data.identite.* in pages/index.js)
// is completely unaffected; these fields are additive, read by the garden
// list/card renderer ONLY as a fallback when data.identite is absent.
// preferredCommonNameFr/En come from gardenApi.fetchGardenRows' own
// batched plant_common_names lookup (never present when inserting via
// addPlant below, where they're simply undefined -> null, harmless since
// only ai_identification rows go through that path in practice).
function rowToLocalPlant({ plant, imageUrl }, plantationTypes, usageTypes) {
  return {
    ...
    source: plant.source ?? "ai_identification",
    commonName: plant.common_name ?? null,
    latinName: plant.latin_name ?? null,
    family: plant.family ?? null,
    category: plant.category ?? null,
    catalogPlantId: plant.catalog_plant_id ?? null,
    taxonId: plant.taxon_id ?? null,
    catalogPlantType: plant.catalog_plant_type ?? null,
    preferredCommonNameFr: plant.preferred_common_name_fr ?? null,
    preferredCommonNameEn: plant.preferred_common_name_en ?? null,
  };
}
...
export function useGarden(user, authLoading, plantationTypes, usageTypes) {
  ...
  const addPlant = useCallback(
    async (localPlantDraft) => {
      if (!user) {
        if (jardin.length >= 10) return { error: FREE_GARDEN_LIMIT_REACHED };
      ...
      try {
        ...
        return { error: monetizationErrorCode(e) || GENERIC_ERROR };
      }
    },
