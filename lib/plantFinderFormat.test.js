...
test("formatPlantGlance: never repeats plant type or exposure — those already lead the key-stat tiles above", () => {
  const glance = formatPlantGlance(
    { plantType: "shrub", evergreen: true, sun: ["full_sun"], floweringMonths: [6, 7, 8], waterNeed: "modéré", containerSuitable: true },
    t,
    "fr"
  );
  assert.equal(glance, "Feuillage persistant · Floraison juin–août · Besoin en eau modéré · Culture en pot");
  assert.ok(!glance.includes("Arbuste"));
  assert.ok(!glance.includes("Plein soleil"));
});

// Regression coverage for the real-iPhone report that "Arbuste · Feuillage
// persistant · Plein soleil" still appeared for Lavandula angustifolia
// despite formatPlantGlance no longer computing a type/exposure segment at
// all. The runtime audit (this round) found no second code path capable of
// producing that string — repo-wide, formatPlantGlance has exactly one
// definition and exactly one call site (pages/plant-finder/[slug].js) — so
// these tests exist as a durable guardrail against it ever regressing back
// in, rather than because a live bug was found in this source.
const LAVANDULA_LIKE = {
  plantType: "shrub",
  evergreen: true,
  sun: ["full_sun"],
...
test("formatPlantGlance: REGRESSION — fr Lavandula-shaped data never includes 'Arbuste' or 'Plein soleil'", () => {
  const glance = formatPlantGlance(LAVANDULA_LIKE, t, "fr");
  assert.ok(glance, "expected a non-null glance for this fixture");
  assert.ok(!glance.includes("Arbuste"), `glance leaked plant type: "${glance}"`);
  assert.ok(!glance.includes("Plein soleil"), `glance leaked exposure: "${glance}"`);
});

test("formatPlantGlance: REGRESSION — en Lavandula-shaped data never includes the plant-type label or 'Full sun'", () => {
  const en = createTranslator("en");
  const glance = formatPlantGlance(LAVANDULA_LIKE, en, "en");
  assert.ok(glance, "expected a non-null glance for this fixture");
  assert.ok(!glance.includes(plantTypeLabel("shrub", en)), `glance leaked plant type: "${glance}"`);
  assert.ok(!glance.includes("Full sun"), `glance leaked exposure: "${glance}"`);
});

// Broader guardrail than the single Lavandula fixture above: exhaustively
// checks every documented plant_type x sun combination (never hardcoding
// just the one species that was reported) — none of their labels, in
// either locale, may ever appear in the synthesis line.
test("formatPlantGlance: REGRESSION — no plant_type or sun label ever leaks in, for any combination, in either locale", () => {
  const en = createTranslator("en");
  for (const type of PLANT_TYPE_VALUES) {
    for (const sunValue of SUN_VALUES) {
      const plant = {
        plantType: type,
        evergreen: true,
        sun: [sunValue],
      ...
      for (const [translator, locale] of [[t, "fr"], [en, "en"]]) {
        const glance = formatPlantGlance(plant, translator, locale);
        const typeLabel = plantTypeLabel(type, translator);
        const sunLabel = sunLabels([sunValue], translator)[0];
        assert.ok(!glance.includes(typeLabel), `[${locale}] glance for type=${type} leaked type label "${typeLabel}": "${glance}"`);
        assert.ok(!glance.includes(sunLabel), `[${locale}] glance for sun=${sunValue} leaked sun label "${sunLabel}": "${glance}"`);
      }
    }
  }
});

test("formatPlantGlance: en locale re-derives the same non-duplicated segments from the same data, no hardcoded fr text", () => {
  const glance = formatPlantGlance(
    { plantType: "shrub", evergreen: true, sun: ["full_sun"], floweringMonths: [6, 7, 8], waterNeed: null, containerSuitable: true },
    createTranslator("en"),
    "en"
  );
  assert.equal(glance, "Evergreen foliage · Flowering June–August · Container growing");
});

test("formatPlantGlance: a single useful secondary item is shown alone, cleanly", () => {
  const glance = formatPlantGlance({ plantType: "tree", evergreen: null, sun: null, floweringMonths: null, waterNeed: null, containerSuitable: true }, t, "fr");
  assert.equal(glance, "Culture en pot");
});

test("formatPlantGlance: only plant type and sun populated (both already in key stats) -> null, block hidden", () => {
  const glance = formatPlantGlance({ plantType: "fern", evergreen: null, sun: ["shade"], floweringMonths: null, waterNeed: null, containerSuitable: null }, t, "fr");
  assert.equal(glance, null);
});

test("formatPlantGlance: evergreen=false and containerSuitable=false are both omitted, not shown as negative claims", () => {
  const glance = formatPlantGlance(
    { plantType: "perennial", evergreen: false, sun: ["partial_sun"], floweringMonths: null, waterNeed: "faible", containerSuitable: false },
    t,
    "fr"
  );
  assert.equal(glance, "Besoin en eau faible");
});

test("formatPlantGlance: missing flowering months -> that segment alone is skipped, others still show", () => {
  const glance = formatPlantGlance({ plantType: "shrub", evergreen: true, sun: ["full_sun"], floweringMonths: null, waterNeed: null, containerSuitable: null }, t, "fr");
  assert.equal(glance, "Feuillage persistant");
});

test("formatPlantGlance: missing water need and container data -> both segments skipped, others still show", () => {
  const glance = formatPlantGlance(
    { plantType: "shrub", evergreen: true, sun: ["full_sun"], floweringMonths: [6, 7, 8], waterNeed: null, containerSuitable: null },
    t,
    "fr"
  );
  assert.equal(glance, "Feuillage persistant · Floraison juin–août");
});
