...
// formatResultCount takes a translator as its last argument (i18n round 1).
...
// R: "Charger plus" pagination cursor
test("R: a valid offset is kept as-is", () => {
  assert.equal(normalizePlantFinderFilters({ offset: 20 }).offset, 20);
});

test("R: missing/non-numeric offset defaults to 0 (first page)", () => {
  assert.equal(normalizePlantFinderFilters({}).offset, 0);
  assert.equal(normalizePlantFinderFilters({ offset: "20" }).offset, 0);
  assert.equal(normalizePlantFinderFilters({ offset: NaN }).offset, 0);
});

test("R: a negative offset is clamped to 0, never forwarded raw", () => {
  assert.equal(normalizePlantFinderFilters({ offset: -5 }).offset, 0);
});

test("R: a fractional offset is floored", () => {
  assert.equal(normalizePlantFinderFilters({ offset: 20.9 }).offset, 20);
});

// N
test("N: result-count pluralization", () => {
  assert.equal(formatResultCount(1, t), "1 plante trouvée");
  assert.equal(formatResultCount(2, t), "2 plantes trouvées");
  assert.equal(formatResultCount(0, t), "0 plantes trouvées");
});
