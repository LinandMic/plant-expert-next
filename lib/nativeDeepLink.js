...
// Trusted external path -> the file the native static export actually
// serves for it. Capacitor's local server runs in html5mode
// (WebViewLocalServer.java): any extensionless path, including
// /reset-password, is answered with index.html, so the WebView would
// render the home page ("__NEXT_DATA__.page" === "/") at that URL.
// Requesting the .html file directly serves the real page. This is an
// INTERNAL mapping only: the trusted external route (and the Android App
// Link path / Supabase redirectTo) stays /reset-password, and a path that
// is not in SUPPORTED_NATIVE_ROUTES is still rejected before this is used
// (so an external ".../reset-password.html" is NOT trusted).
const NATIVE_LOCAL_PAGES = { "/reset-password": "/reset-password.html" };
...
// local static-export path (the .html page for the route, plus query +
// hash, so auth params survive) to hard-navigate the WebView to. Returns
// null for anything untrusted or unsupported — never allow arbitrary
// external navigation inside the WebView.
export function resolveNativeDeepLink(urlString, webOrigin) {
  ...
  const localPage = NATIVE_LOCAL_PAGES[incoming.pathname];
  if (!localPage) return null;

  return { localPath: `${localPage}${incoming.search}${incoming.hash}` };
}
