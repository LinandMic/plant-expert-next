...
// formatFloweringSeason(months, t, locale) -> "juin–août" | "juin" | null
// A compact min–max span built from flowering_months (the min and max month
// PRESENT in the array — not a fabricated "season name"), for the Plant
// Detail "at a glance" synthesis, which needs something shorter than
// formatFloweringMonths' full comma-separated list. French month names are
// lowercased for running text (a grammatical convention, not a new
// translation) while English keeps its usual capitalization — both read
// off the SAME t("format.months") labels every other flowering display
// already uses, never a hardcoded month string.
export function formatFloweringSeason(months, t, locale) {
  if (!Array.isArray(months) || months.length === 0) return null;
  const valid = months.filter((m) => isFiniteNumber(m) && m >= 1 && m <= 12).sort((a, b) => a - b);
  if (valid.length === 0) return null;
  const monthLabels = t("format.months");
  const toDisplay = (label) => (locale === "en" ? label : label.toLowerCase());
  const first = toDisplay(monthLabels[valid[0] - 1]);
  const last = toDisplay(monthLabels[valid[valid.length - 1] - 1]);
  return first === last ? first : `${first}–${last}`;
}

// formatPlantGlance(plant, t, locale) -> "Feuillage persistant · Floraison juin–août · Besoin en eau modéré · Culture en pot" | null
// The Plant Detail "at a glance" synthesis line. Built ONLY from existing
// structured plant_catalog fields — never a freeform/AI-written
// description. Every segment reuses an EXISTING i18n label already shown
// elsewhere on the detail page (finder.evergreen, finder.flowering,
// finder.waterNeed, finder.containerGrowing) — this function only decides
// which already-translated fragments to include and in what order, it
// never invents new descriptive wording. water_need itself is a free-text
// plant_catalog column (no fr/en-coded vocabulary exists for it — see
// finder.waterNeed's own pre-existing raw-value display elsewhere on this
// page), so it is shown verbatim exactly as already stored, same as every
// other reader of that field. A field that is null/absent/false is simply
// omitted (never "N/A", never a placeholder segment) — the whole synthesis
// legitimately returns null when every ingredient is missing.
//
// Deliberately excludes plant_type and sun/exposure (round 2: "mobile
// polish" — real-device testing found these two duplicated the key-stat
// tiles rendered immediately above, e.g. "Arbuste · ... · Plein soleil"
// repeating "Arbuste"/"Plein soleil" verbatim). Height/spread were never
// included here either, for the same reason. This synthesis now only ever
// carries information NOT already shown in the key stats: evergreen,
// flowering season, water need, container suitability.
export function formatPlantGlance(plant, t, locale) {
  if (!plant) return null;
  const segments = [];

  if (plant.evergreen === true) segments.push(t("finder.evergreen"));

  const season = formatFloweringSeason(plant.floweringMonths, t, locale);
  if (season) segments.push(`${t("finder.flowering")} ${season}`);

  if (plant.waterNeed) segments.push(`${t("finder.waterNeed")} ${plant.waterNeed}`);

  if (plant.containerSuitable === true) segments.push(t("finder.containerGrowing"));
