...
// formatPlantGlance(plant, t, locale) -> "Arbuste · Feuillage persistant · Plein soleil · Floraison juin–août · Besoin en eau modéré" | null
// The Plant Detail "at a glance" synthesis line. Built ONLY from existing
// structured plant_catalog fields (plant_type, evergreen, sun, flowering_
// months, water_need, container_suitable) — never a freeform/AI-written
// description. Every segment reuses an EXISTING i18n label already shown
// elsewhere on the detail page (finder.evergreen, the same sun labels as
// the exposure stat, finder.flowering, finder.waterNeed,
// finder.containerGrowing) — this function only decides which
// already-translated fragments to include and in what order, it never
// invents new descriptive wording. water_need itself is a free-text
...
export function formatPlantGlance(plant, t, locale) {
  ...
  const typeLabel = plantTypeLabel(plant.plantType, t);
  if (typeLabel) segments.push(typeLabel);

  if (plant.evergreen === true) segments.push(t("finder.evergreen"));

  const exposureLabels = sunLabels(plant.sun, t);
  if (exposureLabels) segments.push(exposureLabels.join(", "));
