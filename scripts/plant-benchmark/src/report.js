...
**\`plan_restricted\` vs \`unresolved_under_plan\` — distinction obligatoire, jamais confondue** : \`plan_restricted\` signifie qu'une restriction d'abonnement a été explicitement démontrée par la réponse du fournisseur (HTTP 429 + message d'upgrade) **sur un candidat dont la correspondance avec la cible interrogée était elle-même confiante** (\`exact_scientific_match\`/\`exact_cultivar_match\`) — jamais sur un simple candidat apparenté (cultivar différent, correspondance de genre, \`parent_taxon_match\`/\`ambiguous\`) : un 429 sur une fiche qui n'était de toute façon pas la cible ne prouve rien sur la cible elle-même (corrigé — cas réels : \`Viburnum tinus\`→\`Viburnum tinus 'Lisarose'\`, \`Hosta\`→\`Hosta 'Abby'\`, \`Miscanthus sinensis\`→\`Miscanthus sinensis 'Autumn Light'\`, \`Malus domestica\`→un cultivar Goldrush). \`unresolved_under_plan\` couvre deux cas distincts, tous deux "aucune réponse confiante et complète pour cette cible sous ce plan" : (1) une recherche Perenual vide (HTTP 200, \`data: []\`) alors que le niveau d'accès configuré (\`PERENUAL_ACCESS_TIER=personal\`) est documenté par Perenual comme limité à un sous-ensemble du catalogue ("Species Data 1-3000") — l'absence réelle n'est pas démontrable ; (2) un 429 obtenu sur un candidat qui n'était lui-même qu'apparenté à la cible, jamais confidemment identifié comme elle. \`unresolved_under_plan\` n'est jamais compté comme plante trouvée, jamais comme absence botanique certaine, et jamais comme trait manquant chez le fournisseur — comme \`plan_restricted\`, il est exclu du dénominateur de couverture conditionnelle des traits.
